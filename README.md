...
| 3 | Vérification par module (autovalidation R1–R8 + porte de contrôle Q1–Q10 de l'architecture §G.1) | ✅ **M01 → M11 au vert** en mode `--strict` (0 avertissement) · **fiches de contrôle Q1–Q10 publiées** : `05_livrables/fiche_controle_M01_M02.md`, `05_livrables/fiche_controle_M03.md`, `05_livrables/fiche_controle_M05.md` (9/10 vert, 5 écarts), `05_livrables/fiche_controle_M06.md` (10/10 vert, 0 écart bloquant), `05_livrables/fiche_controle_M07.md` (10/10 vert, 2 nuances déclarées), `05_livrables/fiche_controle_M08.md` (10/10 vert, 2 nuances déclarées), `05_livrables/fiche_controle_M09.md` (10/10 vert, 2 nuances déclarées), `05_livrables/fiche_controle_M10.md` (10/10 vert, 3 nuances déclarées), `05_livrables/fiche_controle_M11.md` (10/10 vert, 4 nuances déclarées) · `05_livrables/fiche_controle_M12.md` (10/10 vert, 4 nuances déclarées) · `05_livrables/fiche_controle_M13.md` (10/10 vert, 5 nuances déclarées) |
| 4 | Assemblage, glossaire, index | ⏳ assemblage par module disponible (`render.py --join`) |
| 5 | Composition du manuel PDF (A4, couverture, mise en page éditeur) | ✅ moteur au point — `05_livrables/M01.pdf` (**117 p.**), `M02.pdf` (**122 p.**), `M03.pdf` (**126 p.**), `M04.pdf` (**92 p.**), `M05.pdf` (86 p.), `M06.pdf` (**69 p.**, budget 80 ±15 % = 68-92, OK), `M07.pdf` (**109 p.**, budget 117 ±15 % = 99-135, OK), `M08.pdf` (**119 p.**, budget 117 ±15 % = 99-135, OK), `M09.pdf` (**82 p.**, budget 92,5 ±15 % = [79, 106], OK) `M10.pdf` (**106 p.**, budget 105 ±15 % = [89, 121], OK) `M11.pdf` (**97 p.**, budget 105 ±15 % = [89, 121], OK — projet et évaluation joints, **9** fichiers), `M12.pdf` (**82 p.**, budget 92 ±15 % = [78, 106], OK — **8** fichiers joints) et `M13.pdf` (**107 p.**, budget 105 ±15 % = [89, 121], OK — **9** fichiers joints) ; les HTML de contrôle, régénérables en 20 s, ont été supprimés le 24/09/2026 pour tenir le quota de l'atelier (165,0 → 127,0 Mo) — les paginations M01-M04 et M08 ont été mesurées **après** le correctif de composition du 20/09/2026 (`img { max-width: 100 % }` : les planches 780 px n'étaient plus rognées à droite) |
...
## Dernier module livré — M13 *Modélisation des données* (25/09/2026)

**7 chapitres · 30 h · N3 → N4 · budget 105 p. [89, 121].** Le module transforme un fichier plat de
**240 000** lignes en un modèle de **14** tables (**7** dimensions, **7** faits) dont la recette est
prouvée au franc près (**15 595 154 955** FCFA). Les **7** chapitres sont poussés : **C01** (entités,
attributs, relations, cardinalités), **C02** (1FN, 2FN, 3FN et les **4** anomalies), **C03** (grain,
additivité, tables de faits — les **3** faux totaux mesurés), **C04** (étoile, dimensions conformes,
changements lents types **0** à **3**), **C05** (flocon, table de pont, dimension déchet, faits
multiples), **C06** (temps et calendrier — **1 339** jours, **6** pièges de dates), **C07** (qualité,
documentation, revue en **15** points). **47 245** mots, **107** pages, **394** clés recalculées,
fiche de contrôle **10/10**, PDF `05_livrables/M13.pdf`.

* Chiffres du module : une clé trop large multiplie le chiffre d'affaires par **44,0** ; une clé trop
  fine le divise par **0,36** ; une table de pont qui recopie le montant du ticket donne
  **31 298 080 184** FCFA au lieu de **15 595 154 955** (facteur **2,01**) · **16** libellés de
  familles ramenés à **7** par `TRIM` · l'attribut lu au moment du fait contre le même lu aujourd'hui
  donne **9,1 %** d'écart de prix moyen · une borne de comparaison mal choisie vaut **44,3** points
  (**− 28,9 %** contre **+ 15,4 %**).
* Matériel du projet : `03_exercices/dossier_M13/` — `socle_m13.sql` (**315** lignes dont **99** de
  commentaire, **12** tables, **0** clé déclarée), `modele_fautif.sql` (**3** modèles qui s'exécutent
  sans erreur et donnent **+ 43,0 %**, **+ 89,0 %** et **44** fois le bon chiffre),
  `revue_modele.md` (**5** défauts annotés), `mouvements_clients.csv` (**1 120** mouvements),
  `tarifs_produits.csv` (**616** versions de tarifs).
* Publication : plan n° **58**, socle n° **59**, C01 n° **61**, C02 n° **63**, C03 n° **69**, C04
  n° **72**, C05 n° **74**, C06 n° **77**, C07 n° **78**, clôture n° **80** —
  `05_livrables/journal_pushs.md`.

## Module précédent — M12 *Fondamentaux de la Business Intelligence* (25/09/2026)
